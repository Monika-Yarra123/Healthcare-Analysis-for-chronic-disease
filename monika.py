...
@st.cache_data
def load_data():
    file_path = "Chronic Diseases.csv"
...
with diabetes_tab:
    ...
    st.info(f"Total Patients with Diabetes Risk Score: {len(patients_with_risk):,}")
    st.markdown("---")
    ...
    if len(diabetes_patients) > 0 and 'county' in diabetes_patients.columns:
        ...
        fig_county = px.bar(
            ...
            title='Top 10 Counties by Diabetic Patients'
        )
...
with cvd_tab:
    ...
    st.info(f"Total Patients with CVD Risk Score: {len(patients_with_risk):,}")
    st.markdown("---")
    ...
    if len(cvd_patients) > 0 and 'county' in cvd_patients.columns:
        ...
        fig_county = px.bar(
            ...
            title='Top 10 Counties by CVD Patients'
        )
        fig_county.update_traces(textposition='outside')
        st.plotly_chart(fig_county, use_container_width=True)
    else:
...
# -----------------------------
    ...
    st.info(f"Total Patients with COPD Risk Score: {len(patients_with_risk):,}")
    st.markdown("---")
    ...
    if len(all_copd_patients) > 0 and 'county' in all_copd_patients.columns:
        ...
        fig_county = px.bar(
            ...
            title='Top 10 Counties by COPD Patients'
        )
        fig_county.update_traces(textposition='outside')
        st.plotly_chart(fig_county, use_container_width=True)
    else:
